...
source: https://libp2p.io/docs/getting-started-javascript/
author: Unknown
excerpt: Learn how to run a js-libp2p node and use the ping protocol
---
...
# Run a js-libp2p node

This is the first in a series of tutorials on working with libp2p's javascript implementation, [js-libp2p](https://github.com/libp2p/js-libp2p). We will walk you through setting up a fully functional libp2p node with some basic functionality, and finally we'll send ping messages back and forth between two peers.

## [Install node.js](#install-node-js)

Working with js-libp2p requires [node.js](https://nodejs.org/) >= v16 for development. If you haven't already, install node using whatever package manager you prefer or [using the official installer](https://nodejs.org/en/download/). We recommend using the latest stable version of node, but anything fairly recent should work fine. If you want to see how low you can go, the current version requirements can always be found at the [js-libp2p project page](https://github.com/libp2p/js-libp2p).
...
## [Configure libp2p](#configure-libp2p)
...
Looking at the [available stream multiplexing](https://github.com/libp2p/js-libp2p/blob/master/doc/CONFIGURATION.md#stream-multiplexing) modules, js-libp2p supports `@chainsafe/libp2p-yamux` and `@libp2p/mplex`, but [mplex](https://libp2p.io/docs/mplex/) is [deprecated](https://github.com/libp2p/specs/issues/553) so we will use [Yamux](https://libp2p.io/docs/yamux/) here. You can install `@chainsafe/libp2p-yamux` and add it to your libp2p node as follows in the next example.
...
## [Lets play ping pong!](#lets-play-ping-pong)
