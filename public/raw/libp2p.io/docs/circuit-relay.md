---
title: "Circuit Relay"
source: https://libp2p.io/docs/circuit-relay/
author: Unknown
excerpt: Circuit relay is a transport protocol that routes traffic between two peers over a third-party relay peer.
---
...
## [What is Circuit Relay?](#what-is-circuit-relay)

Circuit relay is a [transport protocol](https://libp2p.io/docs/transports-overview/) that routes traffic between two peers over a third-party "relay" peer.

In many cases, peers will be unable to [traverse their NAT and/or firewall](https://libp2p.io/docs/nat-overview/) in a way that makes them publicly accessible. Or they may not share common [transport protocols](https://libp2p.io/docs/transports-overview/) that would allow them to communicate directly.
...
## [Relay addresses](#relay-addresses)

A relay circuit is identified using a multiaddr that includes the [Peer ID](https://libp2p.io/docs/peers/#peer-id) of the peer whose traffic is being relayed (the listening peer or "relay target").
...
The address above is interesting, because it doesn't include any [transport](https://libp2p.io/docs/transports-overview/) addresses for either the peer we want to contact (`QmAlice`) or for the relay peer that will convey the traffic. Without that information, the only chance a peer has of dialing me is to discover a relay and hope they have a connection to me.
...
1.  Node `A` is behind a NAT and/or firewall, e.g. detected via the [AutoNAT service](https://libp2p.io/docs/autonat/).
