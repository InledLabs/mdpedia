...
source: https://libp2p.io/docs/webrtc-browser-connectivity/
author: Unknown
excerpt: Learn how to use js-libp2p to establish a connection between browsers and node.js and learn about libp2p transports and concepts in practice.
---
...
While WebRTC is a solution to peer-to-peer connectivity in the context of browsers. Libp2p encompasses a wider scope with building blocks for constructing peer-to-peer apps that support WebRTC in addition to [other transports, such as QUIC, TCP, WebSocket, WebTransport](https://libp2p.io/docs/browser-connectivity/), and essentially form a mesh topology:

![mesh topology](https://libp2p.io/webrtc-guide/mesh.png)
...
The diagram above illustrates a mesh topology with libp2p, whereby each peer is identified by a [Peer ID](https://libp2p.io/docs/peers/) that is derived from a public key. When you create a new peer libp2p will first create a new public-private key pair, unless you provide one. Each Peer can have multiple addresses depending on the transport protocols it can be dialed with, e.g. WebRTC in the browser, which can also change.

## [Peer-to-peer connections: when two aren't enough to tango](#peer-to-peer-connections-when-two-aren-t-enough-to-tango)
...
![WebRTC connection flow diagram](https://libp2p.io/webrtc-guide/sequence-part-1.png)

The second diagram, which continues from the first, illustrates the SDP handshake via the Circuit Relay:

![WebRTC connection flow diagram](https://libp2p.io/webrtc-guide/sequence-part-2.png)
...
2.  Other approaches to Peer routing and discovery include [DHT FIND\_NODE query](https://github.com/libp2p/specs/blob/master/kad-dht/README.md#dht-operations), [HTTP Delegated Routing](https://specs.ipfs.tech/routing/http-routing-v1/), and [GossipSub Peer Exchange](https://github.com/libp2p/specs/blob/master/pubsub/gossipsub/gossipsub-v1.1.md), though browser peers don't tend to be long-lived enough to appear in the results of the first two.
...
*   [Peer ID](https://libp2p.io/docs/peers/#peer-id)
*   [Multiaddresses (often abbreviated multiaddr)](https://libp2p.io/docs/addressing/)
*   [Libp2p transports](https://libp2p.io/docs/transports-overview/)
*   [GossipSub](https://libp2p.io/docs/pubsub/)
...
## [Step 2: Start the js-libp2p node.js relay](#step-2-start-the-js-libp2p-node-js-relay)
...
## [Step 3: Start js-libp2p in the browser](#step-3-start-js-libp2p-in-the-browser)
...
The `createLibp2p` invocation creates a libp2p peer which has its associated key pair and [Peer ID](https://libp2p.io/docs/peers/#peer-id) with support for the WebSocket , WebTransport and WebRTC transports, as well as the [identify protocol](https://libp2p.io/docs/protocols/#identify). It also uses noise to ensure all connections are encrypted, and yamux as the stream multiplexer for the relayed connection.

🛈

\*\*Why is yamux needed?\*\* You may notice that the above js-libp2p configuration adds yamux as the stream multiplexer. While both WebRTC and WebTransport come with native \[stream multiplexing\](/docs/multiplexing-overview/), yamux is still required for WebSockets to multiplex streams on the circuit relay connection. !\[diagram showing circuit relay\](/webrtc-guide/circuit-relay-diagram.png) When a browser initiates the connection to another browser over a circuit relay, the underlying WebSocket stream to the relay is multiplexed by yamux and encrypted by Noise. In the case of WebTransport it is multiplexed and encrypted natively. Either way, the relayed connection between the two browsers must be encrypted by noise otherwise the relay could eavesdrop on the traffic. Once the relayed connection between the browsers is encrypted, it also requires yamux to create a stream for the signaling protocol.
...
## [Step 4: Connect to the relay from the browser](#step-4-connect-to-the-relay-from-the-browser)
...
![Screenshot showing the multiaddr in the UI](https://libp2p.io/webrtc-guide/connect-to-relay.png)

Now click Connect and the peer should appear in the peer List:

![Screenshot showing the connected peer in the UI](https://libp2p.io/webrtc-guide/connected-to-relay.png)

Congratulations, you have now established a WebSocket connection to the relay.

🛈

In a production application that is deployed and served with TLS, the relay peer needs a stable hostname and a TLS certificate due to \[Secure Contexts\](https://developer.mozilla.org/en-US/docs/Web/Security/Secure\_Contexts). This requires you to use a reverse proxy, e.g. Nginx or Caddy, to handle \[TLS termination\](https://en.wikipedia.org/wiki/TLS\_termination\_proxy).

## [Step 5: Make the browser dialable with Circuit Relay](#step-5-make-the-browser-dialable-with-circuit-relay)
...
![diagram showing circuit relay](https://libp2p.io/webrtc-guide/circuit-relay-diagram.png)
...
### [Testing circuit relay](#testing-circuit-relay)
...
## [Step 6: Set the relay in the browser app as a bootstrap peer](#step-6-set-the-relay-in-the-browser-app-as-a-bootstrap-peer)
...
## [Step 7: Listen on WebRTC and establish a direct connection](#step-7-listen-on-webrtc-and-establish-a-direct-connection)
...
![browsers connected](https://libp2p.io/webrtc-guide/browsers-connected.png)
...
## [Step 8: PubSub peer discovery](#step-8-pubsub-peer-discovery)
...
In libp2p, PubSub is implemented with the [GossipSub protocol](https://libp2p.io/docs/pubsub/), which provides an efficient way for mesh networks to exchange messages.
...
![PubSub Peer discovery](https://libp2p.io/webrtc-guide/pubsub-discovery.png)
...
### [Differences between js-libp2p in Node.js and browser](#differences-between-js-libp2p-in-node-js-and-browser)
...
### [Try the go-libp2p relay with WebTransport](#try-the-go-libp2p-relay-with-webtransport)
...
#### [Ephemeral WebTransport multiaddr](#ephemeral-webtransport-multiaddr)
