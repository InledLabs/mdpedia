---
title: cache – React
source: https://es.react.dev/reference/react/cache
author: Unknown
...
*   `cache` is for use in [Server Components](https://es.react.dev/blog/2023/03/22/react-labs-what-we-have-been-working-on-march-2023#react-server-components) only.
...
This is because cache access is provided through a [context](https://es.react.dev/learn/passing-data-deeply-with-context) which is only accessible from a component.

##### Profundizar

#### When should I use `cache`, [`memo`](https://es.react.dev/reference/react/memo), or [`useMemo`](https://es.react.dev/reference/react/useMemo)?[](#cache-memo-usememo "Link for this heading")
...
In general, you should use [`useMemo`](https://es.react.dev/reference/react/useMemo) for caching a expensive computation in a Client Component across renders. As an example, to memoize a transformation of data within a component.
...
You should use [`memo`](https://es.react.dev/reference/react/memo) to prevent a component re-rendering if its props are unchanged.
