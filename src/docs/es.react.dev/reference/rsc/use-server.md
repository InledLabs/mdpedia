---
title: Directiva 'use server' – React
source: https://es.react.dev/reference/rsc/use-server
author: Unknown
...
Añade `'use server'` al principio del cuerpo de una función asíncrona para marcar la función como invocable desde el cliente. Llamamos a estas funciones [_Server Functions_](https://es.react.dev/reference/rsc/server-functions).
...
*   Para importar Server Functions desde [código cliente](https://es.react.dev/reference/rsc/use-client), la directiva debe usarse a nivel de módulo.
...
*   Las Server Functions deben llamarse en una [Transición](https://es.react.dev/reference/react/useTransition). Las Server Functions pasadas a [`<form action>`](https://es.react.dev/reference/react-dom/components/form#props) o [`formAction`](https://es.react.dev/reference/react-dom/components/input#props) se llamarán automáticamente en una transición.
...
### Consideraciones de seguridad[](#security "Link for Consideraciones de seguridad ")
...
### Argumentos y valores de retorno serializables[](#serializable-parameters-and-return-values "Link for Argumentos y valores de retorno serializables ")
...
*   Elementos de React, o [JSX](https://es.react.dev/learn/writing-markup-with-jsx)
...
Los valores de retorno serializables compatibles son los mismos que las [props serializables](https://es.react.dev/reference/rsc/use-client#passing-props-from-server-to-client-components) para una barrera de Client Component.

## Uso[](#usage "Link for Uso ")

### Server Functions en formularios[](#server-functions-in-forms "Link for Server Functions en formularios ")

El caso de uso más común de las Server Functions será llamar a funciones que mutan datos. En el navegador, el [elemento HTML form](https://developer.mozilla.org/es/docs/Web/HTML/Element/form) es el enfoque tradicional para que un usuario envíe una mutación. Con React Server Components, React introduce soporte de primera clase para Server Functions como Acciones en [formularios](https://es.react.dev/reference/react-dom/components/form).
...
#### Manejo de valores de retorno en formularios[](#handling-return-values "Link for Manejo de valores de retorno en formularios ")
...
Para actualizar la UI en base al resultado de una Server Function mientras se soporta mejora progresiva, usa [`useActionState`](https://es.react.dev/reference/react/useActionState).
...
Ten en cuenta que como la mayoría de los Hooks, `useActionState` solo puede llamarse en [código cliente](https://es.react.dev/reference/rsc/use-client).

### Llamar a una Server Function fuera de `<form>`[](#calling-a-server-function-outside-of-form "Link for this heading")

Las Server Functions son endpoints del servidor expuestos y pueden llamarse desde cualquier lugar en el código cliente.

Cuando uses una Server Function fuera de un [formulario](https://es.react.dev/reference/react-dom/components/form), llama a la Server Function en una [Transición](https://es.react.dev/reference/react/useTransition), lo que te permite mostrar un indicador de carga, mostrar [actualizaciones de estado optimistas](https://es.react.dev/reference/react/useOptimistic), y manejar errores inesperados. Los formularios envolverán automáticamente las Server Functions en transiciones.
