---
title: <link> – React
source: https://es.react.dev/reference/react-dom/components/link
author: Unknown
...
`<link>` supports all [common element props.](https://es.react.dev/reference/react-dom/components/common#props)
...
*   The component that renders `<link>` will [suspend](https://es.react.dev/reference/react/Suspense) while the stylesheet is loading.
...
If a component depends on a certain stylesheet in order to be displayed correctly, you can render a link to that stylesheet within the component. Your component will [suspend](https://es.react.dev/reference/react/Suspense) while the stylesheet is loading. You must supply the `precedence` prop, which tells React where to place this stylesheet relative to others — stylesheets with higher precedence can override those with lower precedence.

### Nota

When you want to use a stylesheet, it can be beneficial to call the [preinit](https://es.react.dev/reference/react-dom/preinit) function. Calling this function may allow the browser to start fetching the stylesheet earlier than if you just render a `<link>` component, for example by sending an [HTTP Early Hints response](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/103).
