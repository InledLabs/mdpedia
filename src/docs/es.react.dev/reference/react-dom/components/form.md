---
title: <form> – React
source: https://es.react.dev/reference/react-dom/components/form
author: Unknown
...
`<form>` supports all [common element props.](https://es.react.dev/reference/react-dom/components/common#props)
...
Render a `<form>` with an input and submit button. Pass a Server Function (a function marked with [`'use server'`](https://es.react.dev/reference/rsc/use-server)) to the `action` prop of form to run the function when the form is submitted.
...
When `<form>` is rendered by a [Server Component](https://es.react.dev/reference/rsc/use-client), and a [Server Function](https://es.react.dev/reference/rsc/server-functions) is passed to the `<form>`’s `action` prop, the form is [progressively enhanced](https://developer.mozilla.org/en-US/docs/Glossary/Progressive_Enhancement).
...
To learn more about the `useFormStatus` Hook see the [reference documentation](https://es.react.dev/reference/react-dom/hooks/useFormStatus).
...
1.  `<form>` be rendered by a [Server Component](https://es.react.dev/reference/rsc/use-client)
2.  the function passed to the `<form>`’s `action` prop be a [Server Function](https://es.react.dev/reference/rsc/server-functions)
3.  the `useActionState` Hook be used to display the error message

`useActionState` takes two parameters: a [Server Function](https://es.react.dev/reference/rsc/server-functions) and an initial state. `useActionState` returns two values, a state variable and an action. The action returned by `useActionState` should be passed to the `action` prop of the form. The state variable returned by `useActionState` can be used to display an error message. The value returned by the Server Function passed to `useActionState` will be used to update the state variable.
...
Learn more about updating state from a form action with the [`useActionState`](https://es.react.dev/reference/react/useActionState) docs
