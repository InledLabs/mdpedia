...
The [`Dialog`](https://gitlab.gnome.org/GNOME/gnome-shell/tree/main/js/ui/dialog.js) and [`ModalDialog`](https://gitlab.gnome.org/GNOME/gnome-shell/tree/main/js/ui/modalDialog.js) modules contain classes for creating dialogs in GNOME Shell. Extension authors might use these when they would normally use a GTK dialog, which can't be created in the GNOME Shell process.

## `Shell` [​](#shell)

WARNING

The `Shell` import is a C library, available only to GNOME Shell and extensions. These elements are documented here due to an unfixed bug in the official [`Shell` API Documentation](https://gjs-docs.gnome.org/st12), which should be preferred for all other cases.

### `Shell.ActionMode` [​](#shell-actionmode)
...
Parent Class: [`St.Widget`](https://gjs-docs.gnome.org/st12/st.widget)
...
*   `addButton(buttonInfo)` — Add a button to the dialog
    *   buttonInfo (`Object`) — Button properties
        ...
        *   key (`Number`) — Optional [key constant](https://gjs-docs.gnome.org/clutter12-constants/), such as `Clutter.KEY_A` or `Clutter.KEY_space`
...
### `Dialog.MessageDialogContent` [​](#dialog-messagedialogcontent)
...
### `Dialog.ListSection` [​](#dialog-listsection)

Parent Class: [`St.BoxLayout`](https://gjs-docs.gnome.org/st12/st.boxlayout)

A widget for the common case of creating a dialog with a list box of items, such as a dialog for connecting to a wireless network. This is intended to be used with [`Dialog.ListSectionItem`](#dialog-listsectionitem).
...
### `Dialog.ListSectionItem` [​](#dialog-listsectionitem)
...
The `ModalDialog` module includes the primary class for creating dialogs, [`ModalDialog.ModalDialog`](#modaldialog-modaldialog).
...
### `ModalDialog.State` [​](#modaldialog-state)
...
A modal dialog. This class sets up a [`Dialog.Dialog`](#dialog-dialog) layout automatically.
...
*   `new ModalDialog.ModalDialog(params)` — Constructor
    *   params (`Object`) — A dictionary of specific construct properties
        ...
        *   actionMode (`Shell.ActionMode`) — A [`Shell.ActionMode`](#shell-actionmode) (default: `Shell.ActionMode.SYSTEM_MODAL`)
...
*   (`Number`) `state` — A [`ModalDialog.State`](#modaldialog-state) (GObject: read-only)
*   (`Dialog.Dialog`) `dialogLayout` — A [`Dialog.Dialog`](#dialog-dialog) layout (JavaScript: read-only)
