...
To make your extension available of GNOME Shell 40, add `"40"` to the [`shell-version`](https://gjs.guide/extensions/overview/anatomy.html#shell-version) field in `metadata.json`.
...
### Interface XML for Preferences [​](#interface-xml-for-preferences)
...
The template is then loaded in `prefs.js` by specifying the template XML in the widget subclass:
...
Widgets may also be used by loading the interface XML with [`Gtk.Builder`](https://gjs-docs.gnome.org/gtk40/gtk.builder) and retrieving objects by ID:
...
#### Convert and Validate Interface XML [​](#convert-and-validate-interface-xml)
...
### CSS for Preferences [​](#css-for-preferences)
...
#### `show_all()` and `destroy()` [​](#show-all-and-destroy)
...
#### Widgets Tree Navigation [​](#widgets-tree-navigation)
...
#### No `margin-left` and `margin-right` property [​](#no-margin-left-and-margin-right-property)
...
#### draw-value property on GtkScale [​](#draw-value-property-on-gtkscale)
...
#### Get Prefs Window and Resize it [​](#get-prefs-window-and-resize-it)
...
#### Custom Icon Theme [​](#custom-icon-theme)
...
#### `can-focus` property [​](#can-focus-property)
...
#### Adding to the Containers [​](#adding-to-the-containers)
...
.set\_start\_child() and .set\_end\_child()
...
.add\_child\_at\_anchor() and .add\_overlay()
...
`GtkHeaderBar.set_show_close_button()`

`GtkHeaderBar.set_show_title_buttons()`

`GtkHeaderBar.set_custom_title()`

`GtkHeaderBar.set_title_widget()`
...
#### GtkHeaderBar sub title [​](#gtkheaderbar-sub-title)
...
#### Menu and Sub Menu [​](#menu-and-sub-menu)
...
#### menu and submenu elements [​](#menu-and-submenu-elements)
...
icon-size property value

GTK\_ICON\_SIZE\_INHERIT

0

GTK\_ICON\_SIZE\_NORMAL

1

GTK\_ICON\_SIZE\_LARGE
...
#### Show and Hide with Animation [​](#show-and-hide-with-animation)
...
`GtkRevealer.set_reveal_child(false)`
...
`GtkInfoBar.set_revealed(true)`

`GtkInfoBar.set_revealed(false)`
...
#### `Gtk.accelerator_parse()` [​](#gtk-accelerator-parse)
...
`Gtk.RadioButton` no longer exists in GTK4. To have the same behavior you can use [`GtkToggleButton`](https://gjs-docs.gnome.org/gtk40/gtk.togglebutton) with `group` property.
...
### Checking GNOME Shell Version [​](#checking-gnome-shell-version)
...
`ui.main.overview.searchEntry`

Created In

`ui.overviewControls.ControlsManager`
...
`ui.searchController.SearchController`
...
[`ui.overviewControls.ControlsManager`](https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/4a7c5890a8b65068a0620ad6c173274013683348/js/ui/overviewControls.js#L344)

Style Class

`.workspace-thumbnails` as wrapper, `workspace-thumbnail-indicator` as selected workspace thumbnail
...
[`ui.overviewControls.ControlsManager`](https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/ec071ae4c2c747cc432839805d0c3754f647d55a/js/ui/overviewControls.js#L356)
...
[`ui.workspace.Workspace`](https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/4a7c5890a8b65068a0620ad6c173274013683348/js/ui/workspace.js#L1319)

Style Class

`.window-caption` for captions, `.window-close` for close

#### Window Preview Icon [​](#window-preview-icon)
...
[`ui.windowPreview.WindowPreview`](https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/750ade34dae296511c246dc6bb07442120141841/js/ui/windowPreview.js#L125)
...
`ui.main.overview.dash`

Created In

[`ui.overviewControls.ControlsManager`](https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/4a7c5890a8b65068a0620ad6c173274013683348/js/ui/overviewControls.js#L317)
...
`#dash` for the wrapper, `.dash-background` for the background
...
`.icon-grid`, `.apps-scroll-view` as wrapper
