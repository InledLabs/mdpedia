...
source: https://gjs.guide/extensions/upgrading/gnome-shell-42.html
author: Unknown
...
GNOME Shell 42 added support for the [`session-modes`](https://gjs.guide/extensions/overview/anatomy.html#session-modes) field in `metadata.json`.
...
Your extension will be enabled in gdm login screen.

unlock-dialog

Your extension will be enabled (or stays enabled) in unlock dialog.

user

Your extension will be enabled (or stays enabled) when unlock is happening.
...
*   `"session-modes"` defaults to `"user"` when it is not presented in
    
*   `metadata.json`. So you don't need to add `"session-modes"` if you don't need it.
    
*   Only system extensions (located in `/usr/share/gnome-shell/extensions/`) can use `"gdm"` in their session mode.
    
*   Extensions with `"session-modes"` can only pass the EGO reviews if they meet [the review guidelines requirements related to the session modes](https://gjs.guide/extensions/review-guidelines/review-guidelines.html#session-modes).
...
There were no relevant to `extension.js` in GNOME 42.
...
### `fillPreferencesWindow()` [​](#fillpreferenceswindow)
...
### Test Preferences Window in Dark and Light mode [​](#test-preferences-window-in-dark-and-light-mode)
...
### Resize Preferences Window [​](#resize-preferences-window)
...
### Enable Search in Preferences Window [​](#enable-search-in-preferences-window)
...
`Gtk.Window.get_titlebar()` and `Gtk.Window.set_titlebar()` are not supported in Libadwaita.
...
`ui.main.screenshotUI`

Created In

`ui.main._initializeUI()`

Direct Access (UI Group)

`ui.main.layoutManager.screenshotUIGroup`

Created In (UI Group)

`ui.screenshot.ScreenshotUI`

Since there is a screen cast mode, we have new _screen recording indicator_ in panel (`ui.status.remoteAccess.ScreenRecordingIndicator`). This element only shows up while the screenshot is recording.
...
`ui.main.panel.statusArea.screenRecording`

#### Workspace Switcher Popup [​](#workspace-switcher-popup)
