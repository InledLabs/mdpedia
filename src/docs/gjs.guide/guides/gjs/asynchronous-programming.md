---
title: "Asynchronous Programming | GNOME JavaScript"
source: https://gjs.guide/guides/gjs/asynchronous-programming.html
...
# Asynchronous Programming | GNOME JavaScript
...
The [`Promise`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) API is a powerful framework for controlling execution flow in an event loop, while keeping code simple and maintainable.
...
In order to process events in a concurrent fashion, a JavaScript engine needs an event loop. Although GJS is powered by Firefox's [SpiderMonkey](https://spidermonkey.dev/) JavaScript engine, it uses GLib's [Event Loop](https://docs.gtk.org/glib/main-loop.html). The event loop is the foundation of concurrent and asynchronous programming in GJS, so we will cover it in some detail.

If you are writing an application using [`Gio.Application`](https://gjs-docs.gnome.org/gio20/gio.application) or one of its subclasses like `Gtk.Application` or `Adw.Application`, a main loop will be started for you when you call [`Gio.Application.run()`](https://gjs-docs.gnome.org/gio20/gio.application#method-run). If you are writing a GNOME Shell Extension, you will be using the main loop already running in GNOME Shell.
...
Here's one more, slightly more advanced example. In this case, we'll create a input stream for `stdin` (that's the stream you use when you type in a terminal) then use it to create a source that triggers when the user presses `Enter`:
...
Both of these functions return an opaque value, just like a signal connection. This value can be passed to [`GLib.Source.remove()`](https://gjs-docs.gnome.org/glib20/glib.source#function-remove), after which the source will be removed from the main context and the callback will not be invoked.
...
Other event sources, like those created by asynchronous methods in GIO, can not be removed directly. Most operations may be cancelled however, by passing a [`Gio.Cancellable`](https://gjs-docs.gnome.org/gio20/gio.cancellable).

## Promises [​](#promises)

In GJS, a [`Promise`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) is basically an event source that triggers when the `resolve()` or `reject()` functions are invoked. If you are new to `Promise` and `async` in JavaScript, you should review the following articles on MDN:
...
### Traditional Usage [​](#traditional-usage)
...
### Traditional Usage [​](#traditional-usage-1)
...
### Promisify Helper [​](#promisify-helper)

In GJS 1.54 a convenient helper was added as a technology preview, based on the work of Outreachy intern [Avi Zajac](https://llzes.org/). Ultimately, GJS will have seamless support for async functions, but until then you can use the "promisify" helper to automatically create `Promise` wrappers.
...
The "finish" function parameter shouldn't end with "async" and can be omitted.
...
One benefit of GIO's asynchronous methods, is the ability to cancel them after they have started. This is done by using a [`Gio.Cancellable`](https://gjs-docs.gnome.org/gio20/gio.cancellable) object, which can be triggered from any thread.
...
You may pass the same `Gio.Cancellable` object to as many operations as you want, and cancel them all with a single call to `Gio.Cancellable.cancel()`. This can be useful if many operations depend on a single state, such as a GNOME Shell Extension being enabled or disabled.
